...
function SecurePDFViewer({ courseId, fileId }: { courseId: string; fileId: string; fileName: string }) {
  ...
  const renderPage = useCallback(async (pageNum: number) => {
    ...
    // 1. Cancel existing render task to prevent "lagging" behind fast clicks
    ...
    // 2. Optimization for High-DPI (Retina) screens - Sharpens text
    ...
    try {
      ...
      // Silent error for intentionally cancelled tasks
    }
  }, [pdfDoc, scale]);
  ...
  // 3. Zoom Logic: Changed to 0.1 increments (10%)
